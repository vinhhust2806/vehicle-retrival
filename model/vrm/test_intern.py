...
from modules.intern_modeling import CLIP4Clip
...
def get_args(description='Intern on Retrieval Task'):
    parser = argparse.ArgumentParser(description=description)
    parser.add_argument("--do_pretrain", action='store_true', help="Whether to run training.")
    parser.add_argument("--do_train", action='store_true', help="Whether to run training.")
    parser.add_argument("--do_eval", action='store_true', help="Whether to run eval on the dev set.")
    parser.add_argument("--best_model", type=str, default='ckpts/best_model.bin', help='')
...
def eval_epoch(args, model, test_dataloader, device, n_gpu):
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader): # Maybe something went wrong here!!!
            ...
            input_ids, input_mask, segment_ids, video, video_mask, nlw = batch
            # input_ids: batchsize, 1, maxwords   ( text được tokenize )
            # input_mask: batchsize, 1, maxwords  ( chữ là 1, còn lại là 0 )
            # segment_ids: batchsize, 1, maxwords ( all is 0 )
            # video: batchsize, 1, maxframe, 1, 3, 224, 224
            # video_mask: batchsize, 1 , maxframe
            if multi_sentence_:
                # multi-sentences retrieval means: one clip has two or more descriptions.
                b, *_t = video.shape
                sequence_output = model.get_sequence_output(input_ids, segment_ids, input_mask) # batchsize, 1, embedding_size (768)
                ...
                if len(filter_inds) > 0:
                    ...
                    visual_output = model.get_visual_output(video, video_mask)   #batchsize, 1, embedded_dim (768)
    ...
    logger.info('Since the text IDs and video IDs are matched by the index, the scores are not corrected.')
    np.save("./ckpts/best_sim_matrix.npy", sim_matrix)
    logger.info('The VRM VVector (sim_matrix) is saved in ./ckpts/best_sim_matrix.npy.')
    R1 = tv_metrics['R1']
    return R1
...
def main():
    ...
    ## ####################################
    if args.do_eval:
        model = load_model(-1, args, n_gpu, device, model_file=args.best_model)
        if args.local_rank == 0:
